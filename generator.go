...
func generateSeed(a ...string) string {
	return generateMD5Hash(strings.Join(a, ""))
}
...
func generateSignature(data string) map[string]string {
	...
	m["ig_sig_key_version"] = fmt.Sprintf("%d", constants.IGSigKeyVersion)
	m["signed_body"] = fmt.Sprintf(
		"%s.%s", generateHMAC(data, constants.IGSigKey), data,
	)
	return m
}
